...
<a name="down11">[[11]](#up11)</a>: 댕글링 포인터, 와일드 포인터는 허상 포인터와 같은 말이다. 해제된 메모리를 가리키거나 적절한 타입의 유효한 객체를 가리키고 있는 포인터를 뜻하며, 이미 해제된 상태이기 때문에 접근 시 이상 동작을 할 수 있어 널 포인터 상태로 처리해주는 것이 좋다.
...
복잡도가 1~10이면 단순 프로그램이며, 11~20이면 중간 위험도를 가진 다소 복잡한 프로그램, 21~50은 위험성이 높은 복잡한 프로그램이며 51을 넘어가면 테스트가 불가능한 매우 높은 위험성의 프로그램으로 펴악된다.  
